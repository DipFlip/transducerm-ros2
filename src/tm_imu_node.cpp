...
TMSerial::TMSerial() : rclcpp::Node("tm_imu")
{
    ...
    this->declare_parameter("publish_gravity_frame", true);     // Publish gravity-aligned world frame
    this->declare_parameter("acceleration_scale", 9.794);       // Scale to convert from g to m/s^2 (default 9.794)
...
void TMSerial::TimerCallback()
{
    ...
    // Publish msg
    if (this->get_parameter("publish_gravity_frame").as_bool()) {
        PublishTransform();
    }
        ...
        switch(header.cmd){                                 // Step 3.2: header.cmd tells what kind of data is inside the payload.
            ...
            case EP_CMD_Raw_GYRO_ACC_MAG_:{
                ...
                if(EP_SUCC_ == eOD.Read_Ep_Raw_GyroAccMag(&ep_Raw_GyroAccMag)){
                    ...
                    float accel_scale = this->get_parameter("acceleration_scale").as_double();
                    float acc_x = ep_Raw_GyroAccMag.acc[0] * accel_scale;
                    float acc_y = ep_Raw_GyroAccMag.acc[1] * accel_scale;
                    float acc_z = ep_Raw_GyroAccMag.acc[2] * accel_scale;
            ...
            case EP_CMD_COMBO_: {
                ...
                if(EP_SUCC_ == eOD.Read_Ep_Combo(&ep_Combo)){
                    // Accelerometer:
                    float accel_scale = this->get_parameter("acceleration_scale").as_double();
                    float ax = (ep_Combo.ax)*(1e-5f) * accel_scale;      // Convert from g to m/s^2
                    float ay = (ep_Combo.ay)*(1e-5f) * accel_scale;
                    float az = (ep_Combo.az)*(1e-5f) * accel_scale;
                    // Gyroscope:
                    ...
                    // Quaternion in (q1,q2,q3,q4)=(x,y,z,w) format from sensor
                    float q1 = (ep_Combo.q1)*(1e-7f);  // x component
                    float q2 = (ep_Combo.q2)*(1e-7f);  // y component
                    float q3 = (ep_Combo.q3)*(1e-7f);  // z component
                    float q4 = (ep_Combo.q4)*(1e-7f);  // w component
                    // RPY:
                    ...
                    // Quaternion: sensor outputs (x,y,z,w) format, map directly to ROS message
